...
var injectionPatterns = []struct {
	...
	{"обращение к ассистенту", regexp.MustCompile(`(?i)(ассистент|assistant|языков\p{L}* модел|language model|\bai\b|нейросет)`)},
	{"«игнорируй»", regexp.MustCompile(`(?i)(игнорир\p{L}*|забудь\p{L}*|ignore (all|previous|the above|prior)|disregard)`)},
	{"смена роли", regexp.MustCompile(`(?i)(ты теперь|отныне ты|you are now|new instructions|новые (инструкции|указания)|системн\p{L}* (промпт|сообщени)|system prompt)`)},
